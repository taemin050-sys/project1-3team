...
사용법(맥에서, 프로젝트 루트에서):
    python -m src.audit_leakage --combos 1 3
또는 노트북에서:
...
def _source(p: Path) -> str:
    """학습 이미지 경로 → 출처 분류: Kaggle train / AIHub TS(우리증강) / VS(공식 Validation)."""
    s = str(p)
...
def _collect_train_images(combos: list[int], include_vs: bool = False) -> list[Path]:
    ...
    paths = [
        p
        for p in (KAGGLE_DATA / "train_images").glob("*.png")
        if "_index" not in p.name
    ]

    try:
        from src.data.aihub_converter import (
            EXCLUDED_COMBOS,
            _collect_image_dir,
            find_aihub_root,
        )
        ...
        if include_vs:
            vs_root = (
                root.parent / "2.Validation" / "원천데이터" / "경구약제조합 5000종"
            )
            if vs_root.exists():
                paths.extend(
...
def audit(
    ...
    # 출처별 "새는" 학습 이미지 경로 목록 — 호출 측(노트북)에서 이 목록을
    # aihub_coco 등에서 제외하는 데 바로 쓸 수 있도록 전체 리스트로 반환한다
    # (n_near_aihub 등은 개수만 알려줄 뿐 어떤 파일인지는 알려주지 않았음).
    aihub_leak_paths = [s for _, s, _, _ in near_dups if _source(s) == "aihub"]
    kaggle_leak_paths = [s for _, s, _, _ in near_dups if _source(s) == "kaggle"]
    vs_leak_paths = [s for _, s, _, _ in near_dups if _source(s) == "VS"]
    ...
    return {
        ...
        "n_near_vs": n_vs,
        "aihub_leak_paths": aihub_leak_paths,
        "kaggle_leak_paths": kaggle_leak_paths,
        "vs_leak_paths": vs_leak_paths,
        "hist": hist,
