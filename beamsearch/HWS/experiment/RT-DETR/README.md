...
Kaggle 알약 객체 탐지 데이터에 RT-DETR-L을 학습하고, 대회 제출 형식의 CSV를 생성하는 실험입니다.

## 파일

- `rtdetr_l_kaggle.ipynb`: 데이터 준비부터 학습, 추론, 제출 파일 검증까지 실행하는 Kaggle 노트북
...
제출 후보 confidence는 `0.05`, `0.15`, `0.25`로 설정했습니다.
...
confidence에 따라 예측 수가 크게 달라졌으며, 낮은 threshold에서는 이미지당 최대 검출 수까지 후보가 생성됐습니다. 후보별 상세 통계와 YOLO11s 비교는 [결과 문서](../results/README.md)에 정리합니다.

## 산출물

노트북 실행 결과는 `/kaggle/working`에 생성됩니다.

- `rtdetr_l_960_best.pt`: 검증 성능이 가장 좋은 가중치
...
- `submission_rtdetr.csv`: 기본 제출 후보
