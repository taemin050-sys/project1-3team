"""E1 베이스라인 floor (LHK/MPS): 학습(50ep) → 로컬 val mAP@[0.75:0.95] → test 제출 CSV."""
...
SSOT, YD, RUN = LHK / "data/processed", LHK / "data/yolo", LHK / "runs/e1_baseline"
W, H = 976, 1280
m2c = {
    int(k): v
...
if dts:
    ...
    ev.summarize()
    print(
        f"\n>>> 로컬 val mAP@[0.75:0.95] = {ev.stats[0]:.4f} | mAP@0.75 = {ev.stats[1]:.4f}"
    )
...
for p, res in zip(
    ...
    for box, cls, cf in zip(
        ...
        aid += 1
sub = RUN / "submission_e1_baseline.csv"
...
print(f"\n제출 CSV: {sub}")
print(
    f" rows={len(rows)} | images={len({r[1] for r in rows})} (test 842 기대) | cats={len({r[2] for r in rows})} | cat⊆train56: {({r[2] for r in rows}) <= set(m2c.values())}"
)
