...
OUT = paths.PROCESSED / "kaggle_aihub_full_inpaint"
CAP_COMBOS = 100000  # 전체 usable(우리56클래스 포함 조합 전부)
W, H = 976, 1280
...
for combo in sel:
    ...
    for imember in combo_imgs[combo]:
        ...
        if img is None or img.shape[:2] != (H, W):
            continue
        # 비-우리 알약 → 인페인트 마스크(딱딱한 단색 대신 주변 텍스처로 seamless 채움: 엣지·평면 아티팩트 제거)
        anns_this = []
        ...
        for dl, (x, y, bw, bh) in boxes:
            if dl in our:
                ...
                cv2.rectangle(nmask, (x0, y0), (x1, y1), 255, -1)
                n_mask += 1
        if nmask.any():
            nmask = cv2.dilate(
                nmask, np.ones((7, 7), np.uint8)
            )  # 경계 여유(그림자·anti-alias)
            img = cv2.inpaint(img, nmask, 5, cv2.INPAINT_TELEA)
